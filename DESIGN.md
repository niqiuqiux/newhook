...
| 简洁 API | 提供 hook_by_addr / hook_by_symbol / unhook 三个核心接口 |
| UNIQUE 模式 | 每个地址仅支持一个 hook（去掉 SHARED/MULTI 复杂性） |
...
**核心思路: 彻底去除 soinfo 依赖，使用标准 POSIX API 替代**

```
shadowhook 方案                          newhook 替代方案
──────────────────────────────────────   ─────────────────────────────────
nothing.so + soinfo 内存扫描              ✗ 完全去除
  推断 soinfo 结构布局

soinfo::call_constructors hook            ✗ 不 hook linker
  监控 .so 加载事件

soinfo → dl_phdr_info 转换               dl_iterate_phdr() 标准 API
  获取 .so 的加载信息                      直接遍历所有已加载 ELF

延迟 hook (目标 .so 未加载)               ✗ 不支持延迟 hook
  等 .so 加载后自动 hook                    要求目标库已加载 (调用方自行保证)
                                           或提供轮询检测辅助函数

dlclose 自动清理                          ✗ 不自动监控
  监控析构回调                              要求调用方在 dlclose 前 unhook
...
这一设计选择的代价：
1. 不支持"延迟 hook"（目标库未加载时的异步等待）
2. 不自动监控 dlclose（需调用方手动管理生命周期）
3. 仅能解析 `.dynsym`（动态符号表），不支持 `.symtab`（调试符号表）

这些代价在大多数 inline hook 场景中是可以接受的，因为：
- 使用方通常明确知道目标库已加载
- hook 生命周期通常由使用方显式管理
- 需要 hook 的符号几乎都是导出符号（在 .dynsym 中）
...
SHARED/UNIQUE/MULTI 三模式          仅 UNIQUE
Hub 代理链 + TLS 栈                 无 Hub，直接替换
Interceptor (CPU context 拦截)      不支持（简化）
xdl 自定义 ELF 加载器               dl_iterate_phdr + ELF 解析
nothing.so + soinfo 扫描            ✗ 完全去除（静态库可用的关键）
Task 异步任务/延迟hook               ✗ 同步 hook，要求库已加载
linker call_ctors/dtors hook        ✗ 不 hook linker
...
│                    Public API Layer                      │
│                     newhook.h / newhook.c                │
│  newhook_init() / hook_func_addr() / hook_sym_name()    │
│                      / unhook()                          │
└────────────────────────┬────────────────────────────────┘
                         │
         ┌───────────────┼───────────────┐
...
```c
// 初始化 hook 引擎（进程生命周期调用一次）
int newhook_init(void);

// 按地址 hook
//   target_addr : 要 hook 的函数地址
//   new_func    : 替换函数
//   orig_func   : [out] 调用原函数的跳板指针
//   返回 : hook 句柄（非 NULL 成功），NULL 失败
void *newhook_hook_func_addr(void *target_addr, void *new_func, void **orig_func);

// 按符号名 hook
//   lib_name : 库路径 (如 "libtarget.so")，NULL 表示任意库
//   sym_name : 符号名
//   其余参数同上
...
// hook 记录表（用于 unhook 查找）
// 使用简单链表管理所有活跃 hook
typedef struct nh_hook_list_node {
    nh_hook_t                  hook;
    struct nh_hook_list_node  *next;
} nh_hook_list_node_t;

static nh_hook_list_node_t *g_hooks;
```
...
```
g_lock (pthread_mutex_t):
  保护: g_hooks 链表, hook/unhook 操作
...
# 公共头文件
set(PUBLIC_HEADERS include/newhook.h)
...
# 静态库
include $(CLEAR_VARS)
LOCAL_MODULE := newhook_static
LOCAL_SRC_FILES := src/newhook.c src/nh_hook.c src/nh_a64.c ...
LOCAL_C_INCLUDES := $(LOCAL_PATH)/include
LOCAL_CFLAGS := -std=c11 -Wall -Os -fvisibility=hidden
include $(BUILD_STATIC_LIBRARY)

# 动态库
include $(CLEAR_VARS)
LOCAL_MODULE := newhook
LOCAL_SRC_FILES := $(同上)
...
newhook/
├── include/
│   └── newhook.h              # 公共 API 头文件 (~80 行)
├── src/
│   ├── newhook.c              # API 实现, 全局状态管理 (~200 行)
│   ├── nh_hook.c              # 核心 hook/unhook 逻辑 (~300 行)
│   ├── nh_hook.h              # hook 内部数据结构
│   ├── nh_a64.c               # ARM64 指令分析与改写 (~400 行)
│   ├── nh_a64.h               # ARM64 指令类型/常量定义
│   ├── nh_trampo.c            # 可执行内存块管理 (~180 行)
│   ├── nh_trampo.h            # trampo 管理器接口
│   ├── nh_enter.c             # enter 跳板管理 (~50 行)
│   ├── nh_enter.h             # enter 接口
│   ├── nh_island.c            # island 分配管理 (~60 行)
│   ├── nh_island.h            # island 接口
│   ├── nh_symbol.c            # ELF 符号解析 (~250 行)
│   ├── nh_symbol.h            # 符号解析接口
│   ├── nh_safe.c              # 信号安全保护 (~80 行)
│   ├── nh_safe.h              # safe 接口
│   ├── nh_util.c              # 工具函数 (~100 行)
│   ├── nh_util.h              # 工具宏/函数
│   ├── nh_errno.h             # 错误码定义
│   └── nh_log.h               # 日志宏
├── CMakeLists.txt             # CMake 构建 (shared + static)
├── Android.mk                 # NDK 构建
├── newhook.map                # 符号导出脚本
└── DESIGN.md                  # 本设计文档
```
...
int main() {
    ...
    // 方式 1: 按地址 hook
    void *h1 = newhook_hook_func_addr(
        (void *)open, (void *)my_open, (void **)&orig_open);

    // 方式 2: 按符号名 hook
    void *h2 = newhook_hook_sym_name(
        "libc.so", "open", (void *)my_open, (void **)&orig_open);

    // 使用...
    open("/tmp/test", 0);  // 会打印日志，然后调用原始 open
